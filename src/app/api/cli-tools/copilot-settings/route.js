...
const hasERouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "ERouter");
};

const getERouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "ERouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getERouterEntry(config);
...
// POST - Apply ERouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing ERouter entry or append
    const idx = config.findIndex((e) => e.name === "ERouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove ERouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "ERouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "ERouter removed from Copilot config",
    });
  } catch (error) {
