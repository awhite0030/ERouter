...
<div align="center">
  <img src="../images/erouter.png?1" alt="Bảng điều khiển ERouter" width="800"/>
  ...
  **Nhà cung cấp AI Miễn cho OpenClaw.**
  ...
  [🚀 Bắt đầu nhanh](#-quick-start) • [💡 Tính năng](#-key-features) • [📖 Cài đặt](#-setup-guide) • [🌐 Website](https://github.com/awhite0030/ERouter)
</div>

---

## 🤔 Tại sao chọn ERouter?
...
Gói kho lưu trữ này là riêng tư (`erouter-app`), vì vậy việc thực thi nguồn/Docker là đường dẫn phát triển cục bộ dự kiến.
...
### 📺 Hướng dẫn thiết lập hoàn chỉnh - ERouter + Claude Code MIỄN PHÍ
  
[![Thiết lập ERouter + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Xem hướng dẫn từng đầy đủ:**
- ✅ Cài đặt & thiết lập ERouter
...
ERouter hoạt động liền mạch với tất cả các công cụ code AI chính:
...
- Công cụ CLI của bạn gửi định dạng OpenAI → ERouter dịch chuyển → Nhà cung cấp nhận định dạng gốc
...
> Bản thân ERouter **không bao giờ thu phí** bạn bất cứ thứ gì. Bạn chỉ trả tiền trực tiếp cho các nhà cung cấp (nếu sử dụng dịch vụ trả phí).
> 
...
### 📊 Hiểu về Chi phí & Thanh toán của ERouter

**Thực tế Thanh toán ERouter:**

✅ **Phần mềm ERouter = MIỄN PHÍ mãi mãi** (mã nguồn mở, không bao giờ thu phí)  
...
❌ **ERouter không bao giờ gửi hóa đơn** hoặc tính phí thẻ của bạn
...
- **Nhà cung cấp giá rẻ** (GLM, MiniMax): Trả tiền trực tiếp cho họ, ERouter chỉ định tuyến
...
<details>
<summary><b>💳 Tôi có bị ERouter tính phí không?</b></summary>

**Không.** ERouter là phần mềm miễn phí, mã nguồn mở chạy trên máy tính của chính bạn. Nó không bao giờ tính phí bạn bất cứ thứ gì.

**Bạn chỉ trả tiền:**
- ✅ **Nhà cung cấp gói đăng ký** (Claude Code $20/tháng, Codex $20-200/tháng) → Trả tiền trực tiếp cho họ trên website của họ
- ✅ **Nhà cung cấp giá rẻ** (GLM, MiniMax) → Trả tiền trực tiếp cho họ, ERouter chỉ định tuyến yêu cầu của bạn
- ❌ **Bản thân ERouter** → **Không bao giờ tính phí bất cứ thứ gì, ever**

ERouter là một proxy/router cục bộ. Nó không cóẻ tín dụng của bạn, không thể gửi hóa đơn và không có hệ thống thanh toán. Đó là phần mềm hoàn toàn miễn phí.
...
ERouter chỉ định tuyến yêu cầu của bạn đến họ - không có "cạm bẫy" hay thanh toán trong tương lai. Đó là các dịch vụ thực sự miễn phí, và ERouter giúp chúng dễ sử dụng với hỗ trợ dự phòng.

**Lưu ý:** số nhà cung cấp gói đăng ký (Antigravity, GitHub Copilot) có thể có các khoảng thời gian dùng thử miễn phí có thể trở thành trả phí sau này, nhưng điều này sẽ được các nhà cung cấp đó thông báo rõ ràng, không phải ERouter.
...
3. **Sử dụng nhà cung cấp gói đăng ký cuối cùng:**
   ...
   - ERouter giúp tối đa hóa giá trị của chúng thông qua theo dõi hạn mức
...
Cơ chế dự phòng thông minh của ERouter ngăn chặn các khoản phí bất ngờ:
...
**Bạn nắm quyền kiểm soát:** Đặt giới hạn chi tiêu cho mỗi nhà cung cấp trong bảng điều khiển, và ERouter sẽ tuân thủ chúng.
...
**Mẹo Chuyên nghiệp:** Sử dụng Opus cho các tác vụ phức tạp, Sonnet cho tốc độ. ERouter theo dõi hạn mức cho mỗi mô hình!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [từ bảng điều khiển erouter]
...
> **Lưu ý:** OpenClaw chỉ hoạt động với ERouter cục bộ. Sử dụng `127.0.0.1` thay vì `localhost` để tránh các vấn đề phân giải6.
...
| Biến | Mặc định | Mô tả |
|----------|---------|-------------|
| `JWT_SECRET` | Tự động sinh (`~/.erouter/jwt-secret`) | Bí mật ký JWT cho cookie xác thực bảng điều khiển (đặt để chia sẻ giữa nhiều instance) |
| `INITIAL_PASSWORD` | `123456` | Mật khẩu đăng nhập đầu tiên khi không có hash đã lưu tồn tại |
| `DATA_DIR` | `~/.erouter` |ị trí cơ sở dữ liệu ứng dụng chính (`db.json`) |
...
| `CLOUD_URL` | `https://github.com/awhite0030/ERouter` | URL cơ sở endpoint đồng bộ đám mây phía máy chủ |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | URL cơ sở tương thích ngược/công khai (ưu tiên `BASE_URL` cho runtime máy chủ) |
| `NEXT_PUBLIC_CLOUD_URL` | `https://github.com/awhite0030/ERouter` | URL đám mây tương thích ngược/công khai (ưu tiên `CLOUD_URL` cho runtime máy chủ) |
...
- Lịch sử sử dụng và log: `~/.erouter/usage.json` và `~/.erouter/log.txt`, được quản lý bởi `src/lib/usageDb.js`.
- request/translator tùy chọn: `<repo>/logs/...` khi `ENABLE_REQUEST_LOGS=true`.
- Lưu trữ sử dụng hiện tại tuân theo logic đường dẫn `~/.erouter` và độc lập với `DATA_DIR`.
...
- Xác minh `CLOUD_URL` trỏ đến endpoint đám mây dự kiến của bạn (ví dụ: `https://github.com/awhite0030/ERouter`)
...
- Triệu chứng thường xuất hiện trên endpoint đám mây công khai (`https://github.com/awhite0030/ERouter/v1`) cho các lệnh gọi không phát trực tiếp (non-streaming).
...
- Runtime ERouter cục bộ bao gồm dự phòng SSE→JSON cho các lệnh gọi không phát trực tiếp khi upstream trả về `text/event-stream`.
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Gửi một yêu cầu tương thích OpenAI trực tiếp đến endpoint đám mây (`https://github.com/awhite0030/ERouter/v1/chat/completions`) với mô hình/key được cung cấp.
...
- Endpoint đám mây công khai (`https://github.com/awhite0030/ERouter/v1/chat/completions`):
...
- **GitHub**: [github.com/decolua/9](https://github.com/awhite0030/ERouter)
...
Cảm ơn tất cả những người đã đóng góp giúp ERouter tốt hơn!
...
[![ Chart](https://starchart.cc/awhite0030/ERouter.svg?variant=adaptive)](https://starchart.cc/awhite0030/ERouter)
...
Xem [Pull Requests](https://github.com/awhite0030/ERouter/pulls) để biết hướng dẫn chi tiết.

---

## 🔀 Forks

**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Một fork TypeScript đầy đủ tính năng của ERouter. Thêm 36+ nhà cung cấp, tự động dự phòng 4 tầng, API đa phương thức (hình ảnh, embedding, âm thanh, TTS), circuit breaker, bộ nhớ đệm ngữ nghĩa, đánh giá LLM và bảng điều khiển được tinh chỉnh. 368+ bài kiểm tra đơn vị. Có sẵn qua npm và.
