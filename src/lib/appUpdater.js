...
// Collect PIDs of all erouter-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from erouter install dir or wrapper cli.js
        const isAppProcess = lower.includes("erouter") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("erouter") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "erouter");
  }
  return path.join(os.homedir(), ".erouter");
}
...
// Resolve npx/erouter binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx erouter` — works regardless of global bin path changes after npm i -g
