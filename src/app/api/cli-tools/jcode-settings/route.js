...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-erouter.env");
};
...
const hasERouterConfig = (config) => {
  ...
  if (providers["erouter"]) return true;
...
export async function GET() {
  ...
  const hasERouter = hasERouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_EROUTER_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile erouter",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["erouter"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_EROUTER_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "erouter configuration removed from jcode",
    });
  } catch (error) {
