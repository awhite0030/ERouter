...
Ejecuta ERouter en tu máquina local para desarrollo y uso personal.

---

## 📦 Instalación

Instala ERouter globalmente vía npm:
...
Inicia ERouter con un solo comando:
...
Presiona `Ctrl+C` en la terminal donde ERouter se está ejecutando.

```bash
# En la terminal ejecutando erouter
^C  # Presiona Ctrl+C
```
...
```bash
# Respaldo
cp -r ~/.erouter ~/.erouter.backup

# Restaurar
cp -r ~/.erouter.backup ~/.erouter
```
