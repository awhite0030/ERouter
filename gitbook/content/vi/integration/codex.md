...
Tích hợp ERouter với OpenAI Codex CLI để định tuyến request API OpenAI qua hệ thống routing thông minh của ERouter.
...
ERouter cung cấp các model Codex sau:
...
1. Xác minh API key đúng trong ERouter dashboard
...
1. Xác minh tên model khớp với cấu hình ERouter
2. Kiểm tra kết nối provider OpenAI đang hoạt động trong ERouter dashboard
...
Đảm bảo bạn đã cấu hình API key trong ERouter cloud dashboard.
