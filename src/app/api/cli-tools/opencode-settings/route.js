...
const hasERouterConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["erouter"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["erouter"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasERouter: hasERouterConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("erouter/") ? config.model.replace(/^erouter\//, "") : null,
...
// POST - Apply ERouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_erouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing erouter provider entry and its models
    const existingProvider = config.provider["erouter"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["erouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `erouter/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `erouter/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("erouter/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove ERouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["erouter"]?.models) {
      delete config.provider["erouter"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["erouter"].models).length === 0) {
        delete config.provider["erouter"];
        if (config.model?.startsWith("erouter/")) delete config.model;
      } else if (config.model === `erouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["erouter"].models);
        config.model = `erouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire erouter provider
      if (config.provider) delete config.provider["erouter"];
      if (config.model?.startsWith("erouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("erouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "ERouter settings removed from OpenCode",
    });
  } catch (error) {
