...
// Check if settings has ERouter config
const hasERouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["erouter"];
};

// Read per-agent models.json and return current model id (without "erouter/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["erouter"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasERouter: hasERouterConfig(settings),
...
// POST - Update ERouter settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `erouter/${model}`;

    // Remove all old erouter/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("erouter/"))
    ...
    // Add fresh erouter models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`erouter/${m}`] = {};
    });

    // Remove old erouter model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("erouter/")) {
    ...
    // Update models.providers.erouter with all models
    settings.models.providers["erouter"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `erouter/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove ERouter settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove ERouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["erouter"];
    ...
    // Remove erouter models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("erouter/"));
    ...
    // Reset agents.defaults.model.primary if it uses erouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("erouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "ERouter settings removed successfully",
    });
  } catch (error) {
