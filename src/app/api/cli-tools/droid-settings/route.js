...
// Check if settings has ERouter customModels
const hasERouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:ERouter"));
};
...
// POST - Update ERouter customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing ERouter configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:ERouter"));
...
// DELETE - Remove ERouter customModels only (keep other settings)
