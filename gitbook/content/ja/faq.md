...
**ERouterは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**ERouterはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
**はい! ERouterは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.erouter`)
...
**はい、ERouterはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.erouter`(またはカスタム`DATA_DIR`)にローカル保存
- ERouterサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
**破壊的変更:**
- [CHANGELOG.md](https://github.com/awhite0030/ERouter/blob/main/CHANGELOG.md)を確認
- メジャー更新前に`~/.erouter`をバックアップ
- メジャーバージョンの移行ガイドを確認
...
詳細は[CONTRIBUTING.md](https://github.com/awhite0030/ERouter/blob/main/CONTRIBUTING.md)を参照。

---

## さらにヘルプが必要?

- **ドキュメント:** [erouter.com/docs](https://github.com/awhite0030/ERouter/docs)
