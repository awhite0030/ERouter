# ERouter Architecture

_Last updated: 2026-02-06_

## Executive Summary

ERouter is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[ERouter Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.erouter/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.erouter/usage.json`, `~/.erouter/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.erouter/db.json`)
- usage stats: `~/.erouter/usage.json`
- request log lines: `~/.erouter/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[ERouter Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.erouter` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/erouter && npm run build`
- Build Docker image: `cd /root/dev/erouter && docker build -t erouter .`
