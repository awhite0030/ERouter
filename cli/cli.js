...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.erouter/runtime
...
// Identifiers for killAllAppProcesses - only kill erouter specifically
const PROCESS_IDENTIFIERS = [
  'erouter'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "erouter")
    : path.join(os.homedir(), ".erouter");
}
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running erouter/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "erouter" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("erouter") && (cmd.includes("cli.js") || cmd.includes("\\erouter") || cmd.includes("/erouter")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running erouter/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "erouter".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("erouter") && (cmd.includes("cli.js") || cmd.includes("/erouter")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 ERouter is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 ERouter is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "erouter", "db.json") : path.join(".erouter", "db.json"));
