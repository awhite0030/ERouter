...
async function buildCodexHeader() {
  ...
  const { installed, hasERouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasERouter, settings } = result.data;
  ...
  // Extract ERouter custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:ERouter-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract ERouter provider config
  const provider = settings?.models?.providers?.["erouter"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("erouter/") ? primary.replace("erouter/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasERouter, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use ERouter\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
