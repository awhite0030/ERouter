...
### Rebrand
- Full product rebrand from 9router → **ERouter** (names, packages, CLI, docs).
- New theme palette: electric indigo / cyan on cool slate (light + dark).
- Upstream attribution retained under MIT (original decolua/9router).
- GitHub home: https://github.com/awhite0030/ERouter
...
- **PXPipe**: PXPIPE token saver вЂ” multimodal prompt compression (#2465)
...
- **OpenAI-to-Claude**: unwrap bare {function:{вЂ¦}} tools without parent type (#2473)
...
## Features
- **Thinking**: per-model thinking level picker on provider page вЂ” appends `(level)` suffix to copied model names for forced reasoning effort across all formats (openai, claude, gemini, deepseek, kimi, qwen, zai, minimax, hunyuan, step)
...
## Features
- **Usage**: track cached tokens + correct input/output/cache cost (#2209) вЂ” hodtien
- **Codex**: show reset credit expiry details (#2290) вЂ” Rafli Ahmad Zulfikar
- **NVIDIA**: add new models and capabilities вЂ” decolua
- **ClinePass**: add provider support вЂ” sternelee

## Fixes
- **Usage**: dedupe streaming request-details log entries вЂ” Qin Li
- **Claude**: drop foreign thinking signatures in passthrough вЂ” decolua
- Prevent non-SSE stream pipe crash and cross-IdP account overwrites (#2244) вЂ” KunN-21
- **Kiro**: route IdC auth to regional CodeWhisperer surface (#2297) вЂ” Volodymyr Saakian
- **Kiro**: add Claude Sonnet 5 model support (#2264) вЂ” Edison42
- **Xiaomi-tokenplan**: region selector, key validation, multi-connection (#2251) вЂ” MiQieR
- **Translator**: strict Anthropic content block compliance (#2225) вЂ” Sahrul Ramadhan Hardiansyah
- **Kimchi**: strip reasoning_content echo to bound multi-turn input tokens вЂ” KunN-21
- **Kimchi**: bump User-Agent to kimchi/0.1.40 (#2256) вЂ” Ansh7473
- **Codebuddy-cn**: strip empty tool_calls arrays to preserve reasoning вЂ” zmf
- **Antigravity**: preserve Claude tool delta index (#2223) вЂ” Sutarto Jordan Chrisfivo
- **MITM**: generate root CA on server startup (#2228) вЂ” Sutarto Jordan Chrisfivo

# v0.5.15 (2026-06-29)

## Features
- Add Kimchi OAuth provider вЂ” Nant361
- Refine Qwen vision/video + thinking model patterns вЂ” decolua
- Opt-in Codex auto-ping quota keep-alive вЂ” Emirhan

## Fixes
- **Responses**: handle response.done terminal events (#2142) вЂ” rifuki
- **Headroom**: skip unsafe responses tool history (#2132) вЂ” Sutarto Jordan Chrisfivo
- **Translator**: map mid-conversation system message to user (claudeв†’openai) вЂ” decolua
- **Gemini**: normalize contents to prevent 400 invalid_argument (#2192) вЂ” warelik
- **Gemini**: backfill thoughtSignature + suppress stream done sentinel вЂ” WARELIK
- **Alicode**: preserve cache_control for DashScope providers (#2069) вЂ” Rex
- **Antigravity**: strip deprecated/readOnly/writeOnly from tool schemas вЂ” iletai, Yudhistira-Official
- **CodeBuddy CN**: show bonus packs as one-time, not monthly-replenishing вЂ” whale9820
- **Kiro**: strip leaked <thinking> tags from content stream (#2158) вЂ” hamsa0x7
- **Tray**: make Windows context menu DPI-aware вЂ” Emirhan
- **Kilocode**: expose full gateway catalog in combo model picker вЂ” jellylarper
- **OpenCode**: fix Go GLM вЂ” decolua

# v0.5.12 (2026-06-26)

## Features
- Add token-saver dashboard page вЂ” decolua
- Add bulk delete for provider connections вЂ” teddytkz
- Resolve GitHub Copilot model catalog from upstream вЂ” caiqinzhou
- Add Venice AI provider вЂ” Brokenc0de
- Add Kiro external_idp import for Microsoft SSO (CLIProxyAPI) вЂ” Stevanus Pangau
- Overhaul Blackbox provider catalog + WebUI test support вЂ” suryacagur

## Fixes
- Provider thinking compatibility (DeepSeek/Gemini) вЂ” Mink Nguyen
- Stop double-counting streaming usage at source вЂ” decolua
- Usage logging dedupe to reduce stats churn вЂ” Mink Nguyen
- Prevent non-JSON SSE lines / duplicate [DONE] from breaking clients (PR #2046) вЂ” qianze
- Resolve Gemini TTS models from catalog вЂ” nguyenha935
- Support Kiro IDC (organization) token import вЂ” quanturbo
- Preserve forced streaming for JSON clients (#2031) вЂ” Joseph Yaksich
- Preserve Responses text format (Codex) вЂ” tenglong
- Support Gemini native TTS generateContent endpoint вЂ” nguyenha935
- Add missing zh-CN endpoint key label (i18n) вЂ” weimaozhen
- CodeBuddy: only send reasoning params when client requests reasoning (#2071) вЂ” Rex
- CodeBuddy CN: show one-shot bonus packs as expiring, not monthly-replenishing
- Show custom provider models in combo picker вЂ” Sapto
- Docker: add docker-compose.yml with headroom enabled by default вЂ” nitsuahlabs
- Clarify token diagnostics vs provider billing (headroom, #1998) вЂ” Sutarto Jordan Chrisfivo
- Translate openai-responses input through OpenAI for compression (#1998) вЂ” Ankit
- Kiro: report 1M context window for claude-opus-4.8 вЂ” EdisonPVE
- Avoid stale redirects after auth changes (#2100) вЂ” Emirhan
- Mark Claude Opus 4.7 (dashed id) as 1M context вЂ” Brokenc0de
- Preserve reasoning effort through Codex translations вЂ” ntdung6868
- Token-saver: full width card layout вЂ” decolua
- Antigravity: retry transient upstream failures вЂ” Sutarto Jordan Chrisfivo
- Param-support: handle strip rules without match/drop (#1960) вЂ” Joseph Yaksich
- Translator: resolve custom provider prefix in debug endpoint (#1083) вЂ” hamsa0x7

# v0.5.8 (2026-06-21)

## Features
- **Antigravity**: native image generation support (image models tagged kind:image, hiб»ѓn thб»‹ trong media-providers UI)
...
- **Headroom**: proxy lifecycle management + dashboard UI (one-click start/stop, install detection, status probing, token saver, claudeв†”openai shape conversion)
- **CodeBuddy CN**: new OAuth provider (copilot.tencent.com) вЂ” 15-model catalog, /v2 inference, forced streaming, OpenAI-style reasoning
...
## Features
- **Combo Fusion strategy** вЂ” fans the prompt out to all member models in parallel, then a configurable judge model synthesizes one final answer (quorum-grace, anonymized sources, graceful degradation)
- **Per-combo strategy selector** вЂ” pick `fallback` / `round-robin` / `fusion` / `capacity` per combo (replaces the old round-robin toggle), with a judge picker for fusion
- **Capacity auto-switch** вЂ” reorders models per request so images/PDFs route to capable models first
- **Kiro headless API-key auth** (`ksk_`) + direct `claudeв†”kiro` route that avoids the lossy OpenAI two-hop pivot
- **Claude auto-ping** вЂ” warms the 5h quota window right after reset so a fresh window starts immediately (per-connection toggle)

## Fixes
- **Claude 429**: stop hammering the OAuth usage endpoint вЂ” cache resetAt, throttle quota refresh to 3 min, cool down after a 429 (chat unaffected)
...
- **Executors**: strip params unsupported by the provider/model (drops deprecated `temperature` for claude-opus-4 в†’ Anthropic 400)
- **Translator**: derive deterministic tool_call ids for gemini/antigravity в†’ OpenAI so function call/response pair correctly (fixes tool-pairing 400s)
...
## Internal
- Large **open-sse / translator refactor** (~40 commits): unified provider/model registry (LiteLLM-style `models[]` + `kind` field, 100 co-located registry files), single-sourced media/OAuth/refresh/token URLs, registry-based dispatch for usage & token-refresh, DRY translator concerns (buildUsage, encodeDataUri, finishReasonMap, chunkBuilder, reasoningDeltaвЂ¦), ESM-safe registry init, large-file splits, dead-code removal, and golden/no-regression test gates
...
## Docs
- README: add Indonesian ERouter tutorial video (#1709)
...
## Fixes
- Codex: harden streaming timeouts (stall/connect raised to 60s, configurable per-provider), accept `response.done` event, and always emit a terminal `response.failed` + `[DONE]` for Responses passthrough when a stream closes, stalls, or aborts before a terminal event вЂ” prevents codex clients from hanging (#1648, #1680, #1688, #1618)
...
- Lower stream stall timeout 35s в†’ 30s for faster hang detection
...
## Improvements
- Lower stream stall timeout from 60s в†’ 35s for faster hang detection
...
## Features
- Xiaomi MiMo Token Plan: region selector (Singapore / China / Europe) вЂ” keys are cluster-specific
...
- rtk: Kiro format tool result compression вЂ” handle conversationState.history & currentMessage, preserve error results, ~13.6% savings (#1194)

## Fixes
- openclaw: normalize agent.model object form `{primary, fallbacks}` before .startsWith в†’ fix TypeError & 'not configured' status (#1216)
...
## Breaking Changes
- Tunnel public URL changed вЂ” old tunnel links no longer work, please reconnect to get the new URL