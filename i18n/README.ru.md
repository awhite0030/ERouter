<div align="center">
  <img src="../images/erouter.png?1" alt="Панель управления ERouter" width="800"/>
  
  # ERouter - Free AI Router
  ...
  **Бесплатный AI-провайдер для OpenClaw.**
  ...
  [🚀 Быстрый старт](#-quick-start) • [💡 Возможности](#-key-features) • [📖 Установка](#-setup-guide) • [🌐 Сайт](https://github.com/awhite0030/ERouter)
</div>
...
Пакет этого репозитория приватный (`erouter-app`), поэтому запуск из исходников/Docker — это ожидаемый путь локальной разработки.
...
### 📺 Полное руководство по настройке - ERouter + Claude Code БЕСПЛАТНО
  
[![Настройка ERouter + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Полное пошаговое руководство:**
- ✅ Установка и настройка ERouter
...
ERouter бесшовно работает со всеми основными AI-инструментами для кодинга:
...
- Ваш CLI-инструмент отправляет формат OpenAI → ERouter транслирует → Провайдер получает родной формат
...
> Сам ERouter **никогда ничего не взимает** с вас. Вы платите напрямую провайдерам (если используете платные сервисы).
> 
...
### 📊 Понимание затрат и оплаты в ERouter

**Реальность оплаты ERouter:**

✅ **Софт ERouter = БЕСПЛАТНО навсегда** (открытый код, никогда не взимает плату)  
...
❌ **ERouter никогда не выставляет счёт** и не списывает с вашей карты
...
- **Дешёвые провайдеры** (GLM, MiniMax): Платите им напрямую, ERouter только маршрутизирует
- **БЕСПЛАТНЫЕ провайдеры** (iFlow, Kiro, Qwen): Действительно бесплатны навсегда, без скрытых платежей
- **ERouter**: Никогда ничего не взимает, никогда
...
Панель отслеживает ваше использование токенов и показывает **оценочные затраты**, как если бы вы напрямую использовали платный API. Это **не реальная оплата** — это справка, показывающая, сколько вы экономите, используя бесплатные модели или существующие подписки через ERouter.
...
<details>
<summary><b>💳 Взимает ли с меня плату ERouter?</b></summary>

**Нет.** ERouter — это бесплатное ПО с открытым кодом, работающее на вашем собственном компьютере. Оно никогда ничего с вас не взимает.

**Вы платите только:**
- ✅ **Провайдерам подписки** (Claude Code $20/мес, Codex $20-200/мес) → Платите им напрямую на их сайте
- ✅ **Дешёвым провайдерам** (GLM, MiniMax) → Платите им напрямую, ERouter только маршрутизирует ваши запросы
- ❌ **Самому ERouter** → **Никогда ничего не взимает, никогда**

ERouter — это локальный прокси/роутер. У него нет вашей кредитной карты, он не может выставлять счета и не имеет платёжной системы. Это полностью бесплатное ПО.
...
ERouter только маршрутизирует ваши запросы к ним — никаких «ловушек» или будущих платежей. Это действительно бесплатные сервисы, а ERouter облегчает их использование с поддержкой резервирования.

**Примечание:** Некоторые провайдеры подписки (Antigravity, GitHub Copilot) могут иметь бесплатные пробные периоды, которые позже становятся платными, но об этом чётко уведомляют сами провайдеры, а не ERouter.
...
3. **Используйте провайдеров подписки в последнюю очередь:**
   ...
   - ERouter помогает максимизировать их ценность через отслеживание квоты
...
Умный механизм резервирования ERouter предотвращает неожиданные расходы:
...
**Вы контролируете:** Установите лимиты расходов на каждого провайдера в панели, и ERouter будет их соблюдать.
...
**Профи-совет:** Используйте Opus для сложных задач, Sonnet для скорости. ERouter отслеживает квоту для каждой модели!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [из панели управления erouter]
...
> **Примечание:** OpenClaw работает только с локальным ERouter. Используйте `127.0.0.1` вместо `localhost`, чтобы избежать проблем с разрешением имён.
...
docker run -d \
  --name erouter \
  -p 20128:20128 \
  --env-file /root/dev/erouter/.env \
  -v erouter-data:/app/data \
  -v erouter-usage:/root/.erouter \
  erouter
```
...
|----------|---------|-------------|
| `JWT_SECRET` | Автогенерация (`~/.erouter/jwt-secret`) | Секрет подписи JWT для cookie аутентификации панели (задайте для общего доступа между инстансами) |
| `INITIAL_PASSWORD` | `123456` | Пароль первого входа при отсутствии сохранённого хеша |
| `DATA_DIR` | `~/.erouter` | Расположение основной БД приложения (`db.json`) |
...
| `CLOUD_URL` | `https://github.com/awhite0030/ERouter` | Серверный базовый URL эндпоинта облачной синхронизации |
...
| `NEXT_PUBLIC_CLOUD_URL` | `https://github.com/awhite0030/ERouter` | Обратно совместимый/публичный облачный URL (приоритет `CLOUD_URL` для серверного рантайма) |
...
- История использования и логи: `~/.erouter/usage.json` и `~/.erouter/log.txt`, управляется `src/lib/usageDb.js`.
- Опциональные логи запросов/транслятора: `<repo>/logs/...` при `ENABLE_REQUEST_LOGS=true`.
- Хранилище использования следует логике пути `~/.erouter` и независимо от `DATA_DIR`.
...
- Убедитесь, что `CLOUD_URL` указывает на ожидаемый облачный эндпоинт (например, `https://github.com/awhite0030/ERouter`)
...
- Симптом обычно появляется на публичном облачном эндпоинте (`https://github.com/awhite0030/ERouter/v1`) для непотоковых (non-streaming) вызовов.
...
- Локальный рантайм ERouter включает резервирование SSE→JSON для непотоковых вызовов, когда upstream возвращает `text/event-stream`.
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Отправляет OpenAI-совместимый запрос напрямую на облачный эндпоинт (`https://github.com/awhite0030/ERouter/v1/chat/completions`) с указанной моделью/ключом.
...
- Публичный облачный эндпоинт (`https://github.com/awhite0030/ERouter/v1/chat/completions`):
...
- **Сайт**: [erouter.com](https://github.com/awhite0030/ERouter)
...
Спасибо всем, кто помогает делать ERouter лучше!

[![Contributors](https://contrib.rocks/image?repo=awhite0030/ERouter&max=100&columns=20&anon=1)](https://github.com/awhite0030/ERouter/graphs/contributors)
...
См. [Pull Requests](https://github.com/awhite0030/ERouter/pulls) для подробных инструкций.

---

## 🔀 Форки

**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Полнофункциональный TypeScript-форк ERouter. Добавляет 36+ провайдеров, авторезервирование на 4 уровнях, мультимодальный API (изображения, embedding, аудио, TTS), circuit breaker, семантическое кеширование, оценку LLM и доработанную панель. 368+ юнит-тестов. Доступен через npm.
