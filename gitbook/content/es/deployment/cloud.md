...
Despliega ERouter en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.erouter` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/erouter
sudo chown $USER:$USER /var/lib/erouter
```
...
Crea `/etc/nginx/sites-available/erouter`:
...
sudo ln -s /etc/nginx/sites-available/erouter /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de ERouter
...
tar -czf erouter-backup-$(date +%Y%m%d).tar.gz /var/lib/erouter

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/erouter-$(date +\%Y\%m\%d).tar.gz /var/lib/erouter
```
...
```bash
# Verificar si ERouter está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/erouter
chmod 755 /var/lib/erouter
```
